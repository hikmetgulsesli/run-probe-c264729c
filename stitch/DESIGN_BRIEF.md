...
- Run Probe turns the user's request into a directly usable web workflow. The first experience must be the actual web product behavior, not a marketing landing page or placeholder demo.
- - FR-001: Build a tiny single-page status utility called Run Probe. It should show three compact status tiles, one manual Refresh button that updates a visible timestamp, and one Ready/Paused toggle. Keep it simple and local-only.
...
- Status Utility - Run Probe

## SCREEN_SPECS
SCREEN_SPEC_1:
- exact_screen_title: Status Utility - Run Probe
